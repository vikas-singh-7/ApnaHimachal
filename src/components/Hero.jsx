...
const Hero = ({ data }) => {
  return (
    <div
      className="md:h-[80vh] max-sm:h-[80vh] max-sm:bg-blue-500 max-sm:text-white   bg-center bg-cover relative md:bg-[url('./images/heroImage.webp')]"
      // style={{ backgroundImage: "url('./images/heroImage.webp')" }}
    >
      <div className="max-sm:h-[30%] md:hidden md:w-[100%] max-sm:w-[100%]  rounded-md p-2">
      ...
      <div className="absolute max-sm:w-[100%]   md:left-[50%] rounded-md md:top-[50%] md:-translate-y-[50%] md:-translate-x-[50%] p-2  md:w-[40%]  flex md:justify-center flex-col md:items-center backdrop-filter backdrop-blur-none transition duration-300 ease-in-out ">
        <h1 className="md:text-[3.5rem] max-sm:text-[2em] md:tracking-tighter md:font-semibold font-[cursive] max-sm:flex max-sm:justify-start">
          {data.name}
        </h1>
        <p className="md:p-2 rounded-md mt-4 text-[1.5rem] font-[cursive] font-light">
          {data.description}
        </p>
